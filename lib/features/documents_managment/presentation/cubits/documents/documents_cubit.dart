...
class DocumentsListCubit extends Cubit<DocumentsListState> {
  ...
  DocumentsListCubit(
    ...
    // GESTISCO LA CONNESSIONE IN CASO AVESSI CERCATO DI SINCRONIZZARMI OFFLINE
    ...
    if (internetCubit.connectivity is InternetDisconnectedState) {
      emit(CompleteFailureState(
          error: 'Controlla lo stato della tua connessione',
          documentsList: {}));
    } else {
    ...
    if (internetCubit.connectivity is InternetDisconnectedState) {
      ...
      localDocumentsRetrieveAttempt.fold((failure) {
        ...
        Either<FailureEntity, Map<int, Document>>
            remoteDocumentsRetrieveAttempt = await documentsRepository
                .getRemoteDocuments(idClinica: currentUser.idClinica);
